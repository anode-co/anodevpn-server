...
const addrForSlot6 = (ctx, slot) => {
  return IpAddr.fromByteArray(
    Buffer.from(
      (
        ctx.cc.baseAddr6 +
        (BigInt(slot) << BigInt(128 - alloc6(ctx))) +
        MARK128
      ).toString(16),
      "hex"
    ).slice(1)
...
const addrForSlot4 = (ctx, slot) => {
  return IpAddr.fromByteArray(
    Buffer.from(
      (
        ctx.cc.baseAddr4 +
        (BigInt(slot) << BigInt(32 - alloc4(ctx))) +
        MARK32
      ).toString(16),
      "hex"
    ).slice(1)
...
const slotForAddr4 = (ctx, addrStr) => {
  const addr = BigInt(
    "0x" + Buffer.from(IpAddr.parse(addrStr).toByteArray()).toString("hex")
  );
...
const syncSessions = (ctx, done) => {
  ...
  nThen((w) => {
    if (!ctx.mut.cjdns) {
      return void fail(w, "cjdns missing");
    }
    ctx.mut.cjdns.IpTunnel_listConnections(
      w((err, ret) => {
        if (err) {
          return void fail(w, err);
        }
        if (ret.error !== "none") {
          return void fail(w, "cjdns replied " + ret.error);
        }
    ...
    .nThen((w) => {
      console.error(
        `syncSessions() Getting [${numbers.length}] connections...`
      );
      let nt = nThen;
      numbers.forEach((n) => {
        nt = nt((w) => {
          ...
          ctx.mut.cjdns.IpTunnel_showConnection(
            n,
            w((err, ret) => {
              if (err) {
                return void fail(w, err);
              ...
              if (ret.outgoing === 1) {
                throw new Error(
                  "Cannot run a VPN server because this node is a VPN client " +
                    "with lease: " +
    ...
    .nThen((w) => {
      ...
      for (const num in connections) {
        ...
        if (!lease) {
          console.error(
            `syncSessions() external ${conn.key} ${conn.ip4Address} ${conn.ip6Address} ${num}`
          );
      ...
      for (const key in ctx.db.leases) {
        ...
        if (conn) {
          ...
          console.error(
            `syncSessions() detected lease for ${key} at connection num ${conn.number}`
          );
          if (conn.ip4Address && ctx.cfg.cfg4) {
            ...
            if (lease.s4 !== conn.s4) {
              ...
              console.error(
                `syncSessions() Warning: change of address for ` +
  ...
  return (o, sig) => {
    if (!sig) {
      ...
      return void complete(
        sess,
        405,
        "date field required and must be a number"
      );
...
const allocate = (sess, pubkey) => {
  ...
  if (sess.ctx.cc.slots4) {
    ...
    while (sess.ctx.db.slotmap4[s4]) {
      s4 = (s4 + 1) % sess.ctx.cc.slots4;
    }
  ...
  if (sess.ctx.cc.slots6) {
    ...
    while (sess.ctx.db.slotmap6[s6]) {
      s6 = (s6 + 1) % sess.ctx.cc.slots6;
    }
  ...
  readJson(
    ...
    needAuth(sess, (o, sig) => {
      ...
      if (!o[PUBKEY]) {
        ...
        sig.pubkey !== o[PUBKEY] &&
        sig.pubkey !== sess.ctx.mut.coordinatorPubkey
      ) {
        return void complete(
          sess,
          403,
        ...
        return void complete(
          sess,
          400,
          `cannot grant a lease because one was manually added`
        );
      } else {
...
const getAddressCount = (prefix, alloc, addrWidth, t) => {
  ...
  if (isNaN(pfx)) {
    throw new Error(`prefix${t} is not in the form of address/prefix`);
  }
  if (pfx < 0 || pfx > addrWidth) {
    throw new Error(`prefix${t} is out of range`);
  }
  // throw if there's a problem
  IpAddr.parse(addrPfx[0]);
  if (alloc < pfx - 4) {
    throw new Error(`prefix${t} is too small for alloc size`);
  }
...
const computedConfig = (cfg) /*:ComputedConfig_t*/ => {
  ...
  if (cfg.cfg6) {
    ...
    const baseAddr6 = BigInt(
      "0x" + Buffer.from(IpAddr.parse(addrPfx[0]).toByteArray()).toString("hex")
    );
...
const cleanup = (ctx /*:Context_t*/, done) => {
    ...
    nt = nt((w) => {
      ...
      if (!ctx.mut.cjdns) {
        ...
        if (ctx.cfg.dryrun) {
          ...
          cjdns.IpTunnel_removeConnection(
            Number(sn.conn),
            w((err, ret) => {
              if (err) {
                return void fail(w, err);
  ...
  nt((w) => {
    for (const d of toDelete) {
      ...
      if (d.lease.s4) {
        delete ctx.db.slotmap4[d.lease.s4];
      }
      if (d.lease.s6) {
        delete ctx.db.slotmap6[d.lease.s6];
      }
...
const main = () => {
  ...
  nThen((w) => {
    ...
    .nThen((w) => {
      ...
      const checkLoop = () =>
        checkcjdns(ctx, 0, () => setTimeout(checkLoop, 10000));
      checkLoop();

      let lastClean = 0;
      const again = () => {
        nThen((w) => {
          if (now() - ctx.mut.lastSync < 36000000) {
            return;
          }
          syncSessions(ctx, w());
        })
      ...
      Http.createServer((req, res) => httpReq(ctx, req, res)).listen(
        Config.serverPort
      );
    });
};
